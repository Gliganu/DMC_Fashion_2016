...
def testVoting():
    size = 50000
    zeroOrOne = np.vectorize(lambda arg: 1. if arg >= 0.5 else 0.)
    yTest = pd.DataFrame(zeroOrOne(np.random.rand(1, size)))
    classifier1 = pd.DataFrame(zeroOrOne(np.random.rand(1, size)))
    classifier2 = pd.DataFrame(zeroOrOne(np.random.rand(1, size)))
    classifier3 = pd.DataFrame(zeroOrOne(np.random.rand(1, size)))
    predictionMatrix = pd.concat([classifier1, classifier2, classifier3])
    votingClassifier = pd.DataFrame.transpose(pd.DataFrame(Toolbox.makeHardVoting(predictionMatrix)))
    Toolbox.getIndividualClassifierScores(yTest, pd.concat([predictionMatrix, votingClassifier]))
